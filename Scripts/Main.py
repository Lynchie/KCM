...
def test(tAll=True,lAll=True,addData=False):
    global TESTALL
    global LOADALL
    global ADDDATA
    global filename
...
def process(arr,classifier,CurrentSong):
    ...
    if TESTALL:
        ...
        CurrentSong.ProcessSong()
        CurrentSong.PrintData()
...
def removeClassifyNote( arr, x, y, classifier, CurrentSong=SC.Song() ):
...
def scale_appropriately( arr ):
    ...
    if arr.shape[0]>1800 or arr.shape[1]>750:
        ...
        scalex = np.ceil(arr.shape[0]/1800)
        scaley = np.ceil(arr.shape[1]/750)
        scale = int(max(scalex,scaley))
